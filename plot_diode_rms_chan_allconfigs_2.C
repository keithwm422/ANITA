...
void plot_diode_rms_chan_allconfigs_2()
{
/*
  double x[96];
  for (int i =0; i<96; i++){
...
// open all 6 config root files
  TFile * _file0= TFile::Open("/home/keith/anitaBuildTool/components/anitaMagicDisplay/macros/diodeRMSruns/runA/SimulatedAnitaEventFile3.root");
...
//Set Branch Address and fill the diode arrays
  t_A->SetBranchAddress("rms",&rms);
TH2F *h1 = new TH2F("rms v chan", "rms v chan", 100, -1, 98, 1000, 0, 1e-10);
//  t_A->SetBranchAddress("rms",&diode_A);
...
long int entries= t_A->GetEntries();
  for(int j =0; j<entries; j++){

  t_A->GetEntry(j);
  h1->Fill((double)(j),rms);
//    t_A->GetEntry(j);
//    t_B->GetEntry(j);
//    t_C->GetEntry(j);
//    t_G->GetEntry(j);
...
*/
  c1->Draw();
  TString filename = Form("diode_rms_channel_configA.png");
  c1->SaveAs(filename);
